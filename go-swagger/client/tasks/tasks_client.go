...
func (a *Client) GetTaskStatus(params *GetTaskStatusParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*GetTaskStatusOK, error) {
	...
	success, ok := result.(*GetTaskStatusOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for getTaskStatus: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
